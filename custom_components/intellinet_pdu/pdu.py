...
class IntellinetPDUApi:
    ...
    async def _request(
        ...
        *,
        ...
        try:
            async with ClientSession(auth=self._auth(), timeout=timeout, connector=connector) as session:
        ...
        for row in rows:
            ...
            if len(values) >= 3 and outlet_index < 8:
                parsed[outlet_index] = OutletConfig(
                    name=(values[0].strip() or ""),
                    turn_on_delay=self._to_int(values[1]),
                    turn_off_delay=self._to_int(values[2]),
                )
                outlet_index += 1

        if not parsed:
            parsed = {idx: OutletConfig(name="") for idx in range(8)}

        self.outlet_config = parsed
        return parsed

    async def set_outlet_state(self, outlet: int, turn_on: bool) -> None:
        params = {f"outlet{outlet - 1}": 1, "op": 0 if turn_on else 1, "submit": "Apply"}
        await self._request("control_outlet.htm", params=params)

    async def power_cycle_outlet(self, outlet: int) -> None:
        params = {f"outlet{outlet - 1}": 1, "op": 2, "submit": "Apply"}
        await self._request("control_outlet.htm", params=params)
